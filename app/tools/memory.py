...
def list_people(agent_name: str | None = None) -> list[str]:
    with get_session() as session:
        if agent_name:
            result = session.run(
                """
...
@tool
def get_user_preferences(agent_name: str | None = None) -> str:
    """Get stored AI behavior preferences (communication style, format, topics to avoid)."""
    with get_session() as session:
        if agent_name:
            result = session.run(
                """
                MATCH (a:Agent {name: $agent_name})-[:HAS_PREFERENCE]->(p:Preference)
            ...
            result = session.run(
                "MATCH (u:User)-[:HAS_PREFERENCE]->(p:Preference) RETURN p.instruction as instruction"
            )
...
@tool
def check_relationship(person_name: str, agent_name: str | None = None) -> str:
    """Get User's relationship with a person: type, sentiment, since when, events."""
    with get_session() as session:
        if agent_name:
            result = session.run(
                """
                MATCH (a:Agent {name: $agent_name})-[:HAS_PERSON]->(p:Person {name: $name})
                OPTIONAL MATCH (u:User)-[r:KNOWS]->(p)
    ...
    with get_session() as session:
        if agent_name:
            session.run(
                """
                MATCH (a:Agent {name: $agent_name})
                MATCH (e:Event {description: $desc, date: $date})
                MERGE (a)-[:HAS_EVENT]->(e)
                """,
                agent_name=agent_name,
                desc=description,
...
@tool
def add_fact(content: str, category: str, agent_name: str | None = None) -> str:
    ...
    with get_session() as session:
        if agent_name:
            ...
            session.run(
                "MATCH (u:User) MATCH (f:Fact {content: $content}) MERGE (u)-[:HAS_FACT]->(f)",
                content=content,
            )
...
@tool
def add_preference(instruction: str, agent_name: str | None = None) -> str:
    ...
    with get_session() as session:
        if agent_name:
            session.run(
                """
                MATCH (u:User)
                MATCH (p:Preference {instruction: $instruction})
                MATCH (a:Agent {name: $agent_name})
                MERGE (u)-[:HAS_PREFERENCE]->(p)
                MERGE (a)-[:HAS_PREFERENCE]->(p)
                """,
            ...
            session.run(
                "MATCH (u:User) MATCH (p:Preference {instruction: $instruction}) MERGE (u)-[:HAS_PREFERENCE]->(p)",
                instruction=instruction,
            )
...
def add_or_update_relationship(
    ...
    with get_session() as session:
        if agent_name:
            session.run(
                """
                MATCH (p1:Person {name: $start})
                MATCH (p2:Person {name: $end})
                MATCH (a:Agent {name: $agent_name})
                MERGE (p1)-[r:KNOWS]->(p2)
                SET r.relation_type = $rel, r.sentiment = $sent
                MERGE (a)-[:HAS_PERSON]->(p1)
                MERGE (a)-[:HAS_PERSON]->(p2)
                """,
                start=start_person,
                end=end_person,
                rel=relation_type,
...
@tool
def update_fact_or_preference(item_id: str, new_value: str, agent_name: str | None = None) -> str:
...
@tool
def delete_memory(item_id: str, agent_name: str | None = None) -> str:
