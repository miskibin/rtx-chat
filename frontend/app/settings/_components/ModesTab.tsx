...
import { useChatStore, type ModeData } from "@/lib/store"
...
export function ModesTab() {
  const {
    ...
    useShallow((s) => ({
      ...
      fetchModesIfStale: s.fetchModesIfStale,
  ...
  const editingModeNameRef = useRef<string | null>(null)

  useEffect(() => {
    editingModeNameRef.current = editingMode?.name ?? null
  }, [editingMode?.name])
  ...
  // Fetch modes/models only when this tab mounts (i.e., when the user opens it)
  useEffect(() => {
    fetchModesIfStale()
    fetchModelsIfStale()
  ...
  const saveMode = async () => {
    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify(modeToSave),
    })
  ...
  const fetchDocuments = useCallback(async (modeName: string) => {
    try {
  ...
  const pollTaskStatus = useCallback(
    async (taskId: string) => {
      const checkStatus = async () => {
        const modeName = editingModeNameRef.current
        if (!modeName || unmountedRef.current) return
        ...
        try {
          ...
          const res = await fetch(`${API_URL}/modes/${modeName}/knowledge/status/${taskId}`, {
            signal: ctrl.signal,
          })
  ...
  const deleteDocument = async (docId: string) => {
    if (!editingMode?.name) return
    try {
      const res = await fetch(`${API_URL}/modes/${editingMode.name}/knowledge/${docId}`, { method: "DELETE" })
      if (res.ok) fetchDocuments(editingMode.name)
    } catch (error) {
  ...
  // Fetch documents when editing mode changes; also stop any ongoing polling.
  useEffect(() => {
    cleanupPolling()
    setUploadProgress(null)
    ...
    if (editingMode?.name) fetchDocuments(editingMode.name)
    else setDocuments([])
  }, [cleanupPolling, editingMode?.name, fetchDocuments])
          ...
          {availableModes.map((m) => (
            <div
              ...
              className={`relative group p-3 rounded-lg border cursor-pointer transition-all hover:border-primary/50 hover:bg-accent/50 ${
                editingMode?.name === m.name ? "ring-2 ring-primary bg-accent border-primary" : "bg-card"
              }`}
            >
          ...
          <div className="max-w-3xl space-y-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                />
                {editingName !== editingMode.name && editingMode.name && <Badge variant="default">Save as new</Badge>}
                {editingMode.is_template && <Badge variant="secondary">template</Badge>}
              </div>
              <div className="flex gap-2">
                ...
                <Button variant="ghost" onClick={() => setEditingMode(null)}>
                  Cancel
                </Button>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      {editingMode.enabled_tools.length} of {allTools.length} enabled
                    </CardDescription>
                  </div>
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" onClick={() => setEditingMode({ ...editingMode, enabled_tools: allTools })}>
                      Enable All
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => setEditingMode({ ...editingMode, enabled_tools: [] })}>
                      Disable All
                    </Button>
                  </div>
                </div>
                        ...
                        {data.tools.map((tool) => (
                          <div
                            ...
                            className={`px-2.5 py-1 text-xs rounded-md cursor-pointer border transition-colors ${
                              editingMode.enabled_tools.includes(tool.name)
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription>
                        Upload documents to give this mode specific knowledge. Use {"{mode_knowledge}"} in the prompt to inject
                        relevant content.
                      </CardDescription>
                    </div>
