...
import { SearchIcon, Trash2Icon, DatabaseIcon, SparklesIcon } from "lucide-react"
  ...
  return (
    <div className={`border-l-4 rounded-r-lg p-4 ${getTypeStyle(type)}`}>
      <div className="flex items-start justify-between gap-2 mb-2">
        <Badge variant="outline" className={`text-xs ${getBadgeStyle(type)}`}>{type}</Badge>
      ...
      {parts.length > 0 && (
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
          {parts.map((p, i) => (
            <span key={i}><span className="font-medium">{p.label}:</span> {p.value}</span>
          ))}
        </div>
      )}
    </div>
...
export default function MemoriesPage() {
  ...
  useEffect(() => { 
    fetch(`${API_URL}/memories`).then(r => r.json()).then(d => setMemories(d.memories || []))
  }, [])
  ...
  const getMemoriesByType = (type: string) => type === "all" ? memories : memories.filter(m => parseMemoryType(m.memory) === type)
      ...
      <div className="flex-1 overflow-auto p-4">
        <div className="mx-auto max-w-2xl space-y-4">
          ...
          <div className="flex gap-2">
            <Input placeholder="Search memories..." value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === "Enter" && loadMemories(search)} className="flex-1" />
